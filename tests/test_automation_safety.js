...
test('all source and mirror github-script bodies pass expression-size safety', () => {
  const names = [
    'codex-gate.yml',
  ...
  for (const name of names) {
    ...
    for (const body of [...githubScriptBodies(source), ...githubScriptBodies(mirror)]) {
      if (Buffer.byteLength(body, 'utf8') >= LARGE_SCRIPT_EXPRESSION_THRESHOLD) largeBlocks += 1;
      assert.equal(largeGithubScriptHasDirectExpression(body), false, name);
    }
  }
  assert.ok(largeBlocks >= 2, 'regression must exercise large source and mirror scripts');
