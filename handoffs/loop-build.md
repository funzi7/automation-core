...
- Validation: tracked workflow YAML, source/mirror parity, every changed github-script block with `node --check`, 42 deterministic Node policy tests, `git diff --check`; actionlint is run when installed.
