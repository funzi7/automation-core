...
- Freshness is never inferred from Git author/committer dates. Review objects and Codex result comments bind directly to the exact SHA; unmarked surfaces count only after the start of the current contiguous head-SHA epoch recorded by an authenticated `github-actions[bot]` marker, so reopen-on-A preserves review while A→B→A starts fresh. Markerless legacy PRs use the capped Actions history only as a fail-closed bootstrap fallback; current PRs do not depend on that repository-wide search. Repointable inline `commit_id` values alone do not waive current-head review.
