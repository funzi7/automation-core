...
- **Two-token read/mutation split (fix #17 Part C):** the step keeps `github-token: AUTOMATION_PAT` — every MUTATION (`pulls.merge` [PAT-authored so the push to main triggers downstream], labels, comments, `deleteRef`, issue-close) stays on the PAT. A `readonly = getOctokit(github.token)` client (via `env.GH_READONLY_TOKEN`) runs the **two reads** (`checks.listForRef` + `repos.listCommitStatusesForRef`) on `GITHUB_TOKEN`. WHY: fine-grained PATs **cannot be granted the Checks permission at all** (no such option in the PAT UI), so `checks.listForRef` on the PAT died `Resource not accessible by personal access token` on the first real candidate (INCIDENT 2); `GITHUB_TOKEN` carries the workflow's declared `checks: read` + `statuses: read`. Permissions block already had both read scopes.
