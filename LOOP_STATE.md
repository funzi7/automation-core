...
- **Does:** scans the default branch for failed runs (13h lookback), opens a `claude-fix` Issue per failure (logs tail + root-cause prompt), upserts the loop labels, nudges ≤3 attempts then escalates to `needs-owner` + Telegram. Skips Issues already escalated. **`IGNORE_WORKFLOWS`** (loop infra that isn't product CI) = CI Doctor, Claude Fixer, Merge Bot, Sync from automation-core, Codex Gate, Codex Auto-Fix, **Codex Backup Fix**, **Claude Fallback Watchdog** — so a failing backup/watchdog run (e.g. a dispatch 403) doesn't get filed as product CI breakage.
