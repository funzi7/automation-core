...
- Freshness is never inferred from Git author/committer dates. Review objects and Codex result comments bind directly to the exact SHA; unmarked surfaces count only after the server-observed start of the current contiguous head-SHA epoch. Each epoch marker is merely an index to an immutable Actions run: consumers accept it only when GitHub proves the referenced run is this repository's `pull_request_target` Codex Gate run, at the stated attempt, associated with the same PR, and derive the SHA/time from that run rather than comment text. Recent PR-run history is combined conservatively with these verified markers, so A→B→A starts fresh while old markers survive the repository-wide search cap. Repointable inline `commit_id` values alone do not waive current-head review.
