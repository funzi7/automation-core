...
### codex-gate.yml — Codex Gate (blocking check)  (sha `92019ec`; P1-only + wait-for-first-review in PR #25)
- **Does:** the `check-codex-status` blocking check. **GREEN requires BOTH:** (a) Codex has **reviewed the current head** — *any* signal (a review object, an inline note, a top-level comment, or a 👍 reaction; "on the head" = `commit_id == head SHA` **or** dated after the latest commit), **and** (b) **no ACTIVE P1** (a P1 marker on the head not yet followed by a later Codex fix Summary). It **BLOCKS** when an active P1 is unresolved **OR** Codex hasn't reviewed the head yet (**pending**). Otherwise GREEN: clean review, P2-only, 👍, or a stale P1 (raised before the latest commit). `codex-p1-acknowledged` = manual override. Same P1 detection as the bridge across the same 3 channels, fully paginated.
- **Wait-for-first-review, NOT approval:** the pending block is the #66/#67-race guard — it stops merge-bot from merging before Codex has weighed in. It is **not** the old stuck-red "approval marker" requirement: ANY Codex signal on the head (even an empty/👍 review) flips it green, and Codex reviews automatically on every push, so it clears in a minute or two (escape hatch if Codex never reviews = the override label, not a timeout). PR #24's bug (👍 + zero comments stayed red) is fixed because the 👍 reaction and the review object now both count as "reviewed".
...
- **Triggers:** `pull_request: [opened, synchronize, reopened]`, `pull_request_review: [submitted, edited, dismissed]`, `pull_request_review_comment: [created, edited, deleted]`, `issue_comment: [created, edited, deleted]`, `workflow_dispatch` (manual re-check, input `pr_number`). Re-evaluates on every Codex post + every push (pending → green on the next event; the old self-rerun loop / `actions: write` permission are gone).
