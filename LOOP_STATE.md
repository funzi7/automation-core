...
- **Single producer + fail-closed publish (fix #14):** the job key is `codex-gate` with **NO `name: check-codex-status`** line — that `name:` would make the JOB-status check ALSO named `check-codex-status`, i.e. TWO same-named check-runs per head (a downstream copy had wrongly added it as a "restore the fallback gate check name" hack; upstream never had it, and the sync now overwrites the downstream copy). So the explicit publish is the ONLY `check-codex-status`. The publish is **no longer cosmetic/fail-soft**: if `checks.create`/`update` throws (a **downgraded `checks:write` token** on a forked/Dependabot run → no required check while merge-bot still requires `check-codex-status`), the job sets `publishFailed` and **`core.setFailed` fires when the verdict blocks OR `publishFailed`** — so `codex-gate` goes red VISIBLY instead of green-with-a-missing-required-check. The explicit check's own conclusion still equals the verdict (🟢/🔴/🟡). No `needs:`/required-check references the job by `check-codex-status` (verified repo-wide), so removing the duplicate is safe. Resolves Codex's sync-PR "Restore the fallback gate check name" finding without re-adding the duplicate. Freshness / P1-P2 detection / concurrency / MAX_ATTEMPTS unchanged.
