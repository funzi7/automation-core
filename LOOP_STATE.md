...
### codex-gate.yml — Codex Gate (blocking check)  (sha `92019ec`; P1-only rewrite in PR #25)
- **Does:** the `check-codex-status` blocking check. **Blocks ONLY on an ACTIVE P1** — a P1 marker dated *after* the latest commit and *not* followed by a later Codex fix Summary. **Everything else is GREEN:** a clean review, P2-only, a 👍, a stale P1 (raised before the latest commit), or no Codex comments at all. `codex-p1-acknowledged` label = manual override. **No "approval marker" required** — absence of an active P1 is green. (This fixed PR #24, which got a 👍 + zero comments yet stayed red and forced a manual merge: the old gate demanded an explicit post-commit Codex signal and the 👍 reaction was never counted.) Consistent with the bridge's P1-only logic; same P1 detection across the same 3 channels (reviews + top-level comments + inline review comments), fully paginated.
...
- **Triggers:** `pull_request: [opened, synchronize, reopened]`, `pull_request_review: [submitted, edited, dismissed]`, `pull_request_review_comment: [created, edited, deleted]`, `issue_comment: [created, edited, deleted]`, `workflow_dispatch` (manual re-check, input `pr_number`). Re-evaluates on every Codex post + every new push.
- **Removed in PR #25:** the wait/self-rerun loop, the post-commit-signal guard, the reactions fetch, and the `actions: write` permission. ⚠️ **Trade-off:** the gate no longer waits for Codex's *first* review — a brand-new PR is green until a P1 lands (re-tighten with a short wait-for-first-review window if the #66/#67 race resurfaces).
