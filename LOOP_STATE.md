...
- **Does:** the `check-codex-status` blocking check. **GREEN requires BOTH:** (a) Codex has **reviewed the current head** — *any* signal (review object, inline note, top-level comment, or 👍); "reviewed the head" is **generous** = `commit_id == head SHA` **or** dated after the latest commit — **and** (b) **no ACTIVE P1**. "Active" is **strict and DATE-ONLY**: a P1 dated *after* the latest commit, not yet followed by a later Codex fix Summary. **`commit_id` is deliberately NOT used for the active-P1 test** — GitHub re-points a still-applicable inline comment's `commit_id` to the new head, so an old (stale) P1 would otherwise falsely read as active and block (this PR's own 03:44 P1 blocked dff656a that way). It **BLOCKS** on an unresolved active P1 **OR** when Codex hasn't reviewed the head yet (**pending**). Otherwise GREEN: clean review, P2-only, 👍, or a **stale P1 (predating the latest commit)**. `codex-p1-acknowledged` = manual override. `latestCommitDate` = the **max** committer date across the PR's commits (not assumed sorted). Same P1 detection as the bridge across the same 3 channels, fully paginated.
